...
const backupInJson = async (batch) => {
  let isSuccess = true;
  try {
    const sanitizedNextLink =
      (batch["@odata.nextLink"]
        ? batch["@odata.nextLink"]
        : "OtherDocuments?$skiptoken=999999999999999999(last)"
      ).replace(/[/?]/g, "-") || "";
    const backupFile = `${backupFolder}/${entity}-${sanitizedNextLink}.json`;
    await fs.writeFile(backupFile, JSON.stringify(batch));
  } catch (error) {
...
const backupAllPages = async () => {
  ...
  do {
    try {
      ...
      const processedBatch = await backupInJson(batch);

      if (!processedBatch) {
        console.error(`Erro ao processar o lote, interrompendo o processo...`);
        break;
      }
